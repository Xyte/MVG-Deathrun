...
include( "cl_placement.lua" )
include( "menutext.lua" )
include( "cl_voice.lua" )
include( "birthday.lua" )
...
function GM:GetScoreboardNameColor( ply )
	...
	if ply:SteamID() ==  "STEAM_0:0:83009373" then return Color(0, 0, 0) end
	-- Conner
	if ply:SteamID() ==  "STEAM_0:0:59637877" then return Color(255, 20, 147) end 
	-- Nameless
...
usermessage.Hook( "chatmsg", function( um )
...
--HALO EFFECT
CreateClientConVar( "deathrun_highlight", 1, true, false )
function GM:PreDrawHalos()
	if GetConVarNumber( "deathrun_highlight" ) == 0 then return end
...
hook.Add("CreateMove","ThirdPerson", function()
...
function RoundCleanUp()
	if GetRoundState() == ROUND_PREPARING then
	print "cleanup."
		game.CleanUpMap()
	end
end
...
local UpDownFunc = { "STEAM_0:0:51692222", "", "" } --Add a steamID to flip them upside down, infinite amount can be added.

    if not table.HasValue(UpDownFunc, LocalPlayer():SteamID() ) then return end
