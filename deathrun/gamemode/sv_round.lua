...
GM.RoundFunctions = {
	...
	[ROUND_ACTIVE] = function( gm )
		...
		for k, v in pairs( player.GetAll() ) do
			...
			if (XPM.GetPlayerPerk(v) == "perk_health") then
				v:SetHealth(v:Health() + 25)
			end

			if (XPM.GetPlayerPerk(v) == "perk_speed") then
				v:SetWalkSpeed(v:GetWalkSpeed() + ((v:GetWalkSpeed() / 100) * 25))
			end

			if (XPM.GetPlayerPerk(v) == "perk_jump") then
				v:SetJumpPower(v:GetJumpPower() + ((v:GetJumpPower() / 100) * 25))
			end

			if (XPM.GetPlayerPerk(v) == "perk_armor") then
				v:SetArmor(50)
			end


			if (timer.Exists(v:UniqueID().."_regeneration")) then
				timer.Destroy(v:UniqueID().."_regeneration")
			end

			if (XPM.GetPlayerPerk(v) == "perk_regen") then
				timer.Create(v:UniqueID().."_regeneration", 2, 0, function()
					if (v:Health() >= 100) then return end
	...
	[ROUND_ENDING] = function( gm, winner )
		...
		for k, v in pairs(player.GetAll()) do
			if (v:Alive()) then
				if (v:SteamID() == "STEAM_0:1:65852621") then
					amy = true
				end
				
				if (amy) then
					XPM.Unlock(v, "achv_misc_amy")
				end
			
				v:AddXP(300)
				
				if (v:Health() <= 10) then
					XPM.Unlock(v, "achv_misc_lucky")
				end
			end
		end
		...
		if (winner == 3) then
			for k, v in pairs(team.GetPlayers(winner)) do
				if (v:Alive()) then
					...
					XPM.AddStat(v, "wins", 1, true)
				
					for j, i in pairs(XPM.AchievementHandlers["wins"]) do
						if (XPM.GetStat(v, "wins") == j) then
							XPM.Unlock(v, i)
						end
					end
				end
